...
//
    ...
    extension UITerminalView {
        ...
        /// Font pinch disabled (remote fit-to-grid hosts): the same step
        /// quantization drives the host's remote-zoom callback instead.
        private func handleRemotePinchZoom(_ gesture: UIPinchGestureRecognizer) {
            guard let onPointerPinchZoom else { return }
            switch gesture.state {
            case .began:
                lastPinchScale = gesture.scale
            ...
            case .changed:
                ...
                TerminalDebugLog.log(
                    .actions,
                    "remote pinch steps=\(steps) scale=\(String(format: "%.3f", gesture.scale))"
                )
                onPointerPinchZoom(steps, gesture.location(in: self))
