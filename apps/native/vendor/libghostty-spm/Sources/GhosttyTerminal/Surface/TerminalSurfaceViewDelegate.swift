...
@MainActor
public protocol TerminalSurfaceClickableFileDelegate: TerminalSurfaceViewDelegate {
...
/// Scrollback viewport metrics (the `terminal.Scrollbar` action). Fires
/// whenever the viewport moves relative to scrollback: scrolling, new
/// output, resize, screen switches. All values are in rows.
public struct TerminalScrollbarMetrics: Sendable {
    /// Total rows in scrollback + active screen.
    public let total: UInt64
    /// Viewport top offset in rows from the start of scrollback.
    public let offset: UInt64
    /// Viewport height in rows.
    public let viewport: UInt64

    /// Whether the viewport is pinned to the live end of the screen.
    public var isAtBottom: Bool { offset + viewport >= total }
}
...
/// Terminal search (libghostty runs the incremental match, scrollback walk,
/// and highlight rendering itself; the host owes it only a find bar and a
/// match counter). Drive the search with binding actions on the surface:
/// `search:<text>` (empty cancels), `navigate_search:next` /
/// `navigate_search:previous`, `start_search`, `end_search`,
/// `search_selection`.
///
/// Note: match counts are collected as part of drawing, so
/// `terminalDidUpdateSearchTotal` is only raised while the surface actually
/// renders — an occluded/detached surface reports nothing.
@MainActor
public protocol TerminalSurfaceSearchDelegate: TerminalSurfaceViewDelegate {
    /// The surface asked the host to show its search UI (`start_search`
    /// binding). `needle` is the initial query, if any.
    func terminalDidRequestStartSearch(needle: String?)
    /// The surface asked the host to hide its search UI (`end_search`).
    func terminalDidRequestEndSearch()
    /// Total number of matches for the active search.
    func terminalDidUpdateSearchTotal(_ total: Int)
    /// Index of the currently selected match (0-based; negative when no
    /// match is selected).
    func terminalDidUpdateSearchSelected(_ selected: Int)
}

/// Direct touch-scroll gesture from UIKit before it is translated into a
/// Ghostty scroll event. Returning true lets a host consume the gesture and
/// prevents the platform view from also scrolling the local surface.
public enum TerminalTouchScrollPhase: Sendable {
    case began
    case changed
    case ended
    case cancelled
...
@MainActor
public protocol TerminalSurfaceTouchScrollDelegate: TerminalSurfaceViewDelegate {
