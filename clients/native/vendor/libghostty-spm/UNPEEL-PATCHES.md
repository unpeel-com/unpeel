...
22. **Wakeups always tick; only rendering is gated** (2026-09-15, Swift
    ...
    calls `ghostty_app_tick` and its `onWakeup` callback unconditionally.
    `TerminalSurfaceCoordinator.requestImmediateTick()` gates only the
    follow-up refresh on attachment and presentation visibility. Ghostty's
    app mailbox is a 64-slot blocking queue that
