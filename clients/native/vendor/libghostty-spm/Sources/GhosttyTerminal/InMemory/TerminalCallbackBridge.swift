...
final class TerminalCallbackBridge {
    ...
    func handleAction(_ action: ghostty_action_s) {
        ...
        case GHOSTTY_ACTION_SCROLLBAR:
            ...
            TerminalDebugLog.log(
                .actions,
                "callback action=scrollbar total=\(scrollbar.total) offset=\(scrollbar.offset) len=\(scrollbar.len)"
            )
            (delegate as? any TerminalSurfaceScrollbarDelegate)?
                .terminalDidUpdateScrollbar(TerminalScrollbarMetrics(
                    total: scrollbar.total,
                    offset: scrollbar.offset,
                    viewport: scrollbar.len
                ))
