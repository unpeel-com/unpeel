        ...
        func handleInputBarKey(_ key: TerminalInputBarKey) {
            ...
            case .paste:
                ...
                if let text = UIPasteboard.general.string, !text.isEmpty {
                    // Explicit paste keeps Ghostty's paste semantics. Ordinary
                    // software-keyboard/IME commits take the direct host path
                    // in TerminalTextInputHandler instead.
                    inputHandler.pasteText(text)
                }
            ...
            case .backspace:
                // Reuse the UITextInput delete path so marked-text/IME
                // deletion, sticky modifiers, and the in-memory byte route all
                // behave exactly like the software keyboard's own backspace.
                deleteBackward()
            }
        }
