...
//
    ...
    extension UITerminalView: UITextInput, UITextInputTraits {
        ...
        /// Secure entry is the only supported way to remove the keyboard's
        /// dictation mic — system inline dictation cannot work against a
        /// terminal (ranged replacements on a byte stream), so hosts that
        /// provide their own dictation opt in via
        /// `systemDictationDisabled`. The other secure-entry effects
        /// (no prediction bar, no autocorrect) are already the terminal's
        /// configured behavior.
        open var isSecureTextEntry: Bool {
            get { systemDictationDisabled }
            set {}
        }

        /// A secure field otherwise makes iOS show the keyboard's "Passwords"
        /// AutoFill key. Declaring an OTP content type while secure suppresses
        /// that key (the field isn't a password), while keeping the mic hidden.
        /// A terminal never wants password/credential AutoFill anyway. Nil when
        /// not secure.
        open var textContentType: UITextContentType! {
            get { systemDictationDisabled ? .oneTimeCode : nil }
            set {}
        }
        ...
        open func insertText(_ text: String) {
            guard !hardwareKeyHandled else {
                ...
                // Software-keyboard Return must be a KEY, not text: sendText
                // wraps text in bracketed-paste markers whenever the remote
                // app enabled mode 2004, and a pasted newline is a literal
                // line break — Enter then silently fails to submit in agent
                // TUIs (0x28 = HID usage for Return).
                if text == "\n" {
        ...
        /// Terminals cannot honor UIKit's replace-as-you-refine dictation
        /// edits (already-sent bytes are gone), which made dictation deliver
        /// only the first hypothesis fragment. Implementing this switches
        /// UIKit to a commit-once flow: interim hypotheses stay in the
        /// (unsent) marked-text preview, and the final transcript arrives
        /// here as a single insert.
        open func insertDictationResult(_ dictationResult: [UIDictationPhrase]) {
            let text = dictationResult.map(\.text).joined()
            TerminalDebugLog.log(
                .input,
                "insertDictationResult text=\(TerminalDebugLog.describe(text))"
            )
            guard !text.isEmpty else { return }
            // inputHandler.insertText clears any marked-text preview before
            // sending, so the hypothesis preedit never double-commits.
            inputHandler.insertText(text)
        }
