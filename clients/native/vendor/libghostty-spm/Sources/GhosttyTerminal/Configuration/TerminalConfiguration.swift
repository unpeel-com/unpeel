...
public enum TerminalConfigCommand: Sendable, Hashable {
    ...
    var renderedLine: String {
        ...
        case let .fontSize(value):
            "font-size = \(value.formatted(.ghosttyNumber.precision(.fractionLength(0 ... 2))))"
        ...
        case let .cursorOpacity(value):
            "cursor-opacity = \(value.formatted(.ghosttyNumber.precision(.fractionLength(0 ... 3))))"
        ...
        case let .minimumContrast(value):
            "minimum-contrast = \(value.formatted(.ghosttyNumber.precision(.fractionLength(0 ... 2))))"

        case let .backgroundOpacity(value):
            "background-opacity = \(value.formatted(.ghosttyNumber.precision(.fractionLength(0 ... 3))))"
...
/// ghostty's config parser only accepts `.` as the decimal separator, so
/// numeric values must never be rendered with the user's locale.
extension FormatStyle where Self == FloatingPointFormatStyle<Double> {
    static var ghosttyNumber: FloatingPointFormatStyle<Double> {
        FloatingPointFormatStyle<Double>(locale: Locale(identifier: "en_US_POSIX"))
    }
}

extension FormatStyle where Self == FloatingPointFormatStyle<Float> {
    static var ghosttyNumber: FloatingPointFormatStyle<Float> {
        FloatingPointFormatStyle<Float>(locale: Locale(identifier: "en_US_POSIX"))
    }
}
