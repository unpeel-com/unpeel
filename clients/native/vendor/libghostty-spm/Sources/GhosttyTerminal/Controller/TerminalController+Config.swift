...
extension TerminalController {
    ...
    func createApp() {
        ...
        let context = TerminalControllerCallbackContext(controller: self)
        let userdata = Unmanaged.passRetained(context).toOpaque()
        callbackContext = context
        retainedCallbackContextAddress = UInt(bitPattern: userdata)
        ...
        guard let createdApp = ghostty_app_new(&runtimeConfig, cfg) else {
            callbackContext = nil
            retainedCallbackContextAddress = nil
            releaseRetainedTerminalControllerCallbackContext(address: UInt(bitPattern: userdata))
            return
        }
