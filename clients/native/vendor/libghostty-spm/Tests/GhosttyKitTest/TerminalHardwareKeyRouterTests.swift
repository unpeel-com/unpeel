...
struct TerminalHardwareKeyRouterTests {
    @Test
    func `routes committed UI text directly for in memory backends`() {
        let session = InMemoryTerminalSession(write: { _ in }, resize: { _ in })
        #expect(
            TerminalCommittedTextRouter.route(
                " ",
    ...
    @Test
    func `routes committed UI text through ghostty for exec backends`() {
        #expect(
            TerminalCommittedTextRouter.route(
                "hello",
                backend: .exec
