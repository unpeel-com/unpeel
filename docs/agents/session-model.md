...
A blank terminal (empty launch command) where the user hand-typed an agent resumes that agent: the recipe is derived from the primary command alias of the runtime the hooks captured (`provider_runtime`) (`session_ops::resume_base_command`). The exception is an agent the user quit back to the shell: the Host stamps `agent_returned_to_shell_at` on a *proven* return to the owned login shell (never on a bare observation timeout, which also fires as a dying Session winds down), and when that stamp is newer than the last hook event the shell comes back instead. A different agent observed in the foreground never inherits the captured conversation.
